...
const cameraController = new CameraController(world, {
  fov: Math.PI / 3,
  lerpRate: 8,
  obstructionBuffer: UNIT * 0.2,
  maxObstructionShift: UNIT * 3,
});
...
function draw() {
  background(18);
  cameraController.applyPerspective();

  player.update(world);
  const cameraInfo = cameraController.update(player.getCamera());
  world.drawPlatforms();
